...
export default function PaperPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <article className="w-full">
      ...
      {/* Content */}
      <div className={viewMode === 'side-by-side' ? 'grid grid-cols-2 gap-4' : ''} style={{ height: 'calc(100vh - 280px)' }}>
        {/* Original — iframe to arxiv */}
          ...
          <div className="border border-border rounded-lg overflow-hidden h-full">
            {viewMode === 'side-by-side' && (
              <div className="text-xs text-muted px-4 py-2 bg-surface border-b border-border font-medium uppercase tracking-wide">
                原文 Original
                <a href={arxivHtmlUrl} target="_blank" rel="noopener noreferrer" className="ml-2 text-accent">↗</a>
              </div>
            )}
            <iframe
              src={arxivHtmlUrl}
        ...
        {/* Translation — rendered markdown/html */}
        {(viewMode === 'translated' || viewMode === 'side-by-side') && (
          <div className="border border-border rounded-lg overflow-auto h-full">
            ...
            <div className="p-6 paper-content"
              dangerouslySetInnerHTML={{ __html: paper.translatedHtml || paper.html || '' }}
            />
          </div>
        )}
      </div>
